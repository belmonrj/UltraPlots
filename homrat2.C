...
void doit(char *handle="pipi", int r7pid=2, int r8pid=1)
{
  ...
  float pt[33], pt1[33], ept[33], ratio[33], ratio1[33], eratio[33], esysratio[33], esysratio1[33];
  ifstream fin;

  if(r8pid==1) fin.open("STARhomrat2/pipi.dat");
  else if(r8pid==3) fin.open("STARhomrat2/kaka.dat");
  else fin.open("STARhomrat2/pbarp.dat");
  for(int i=0; i<14; i++)
    {
  ...
  int N = 30;
  if(r8pid==1) N = 27;
  else if(r8pid==3) N = 16;
  else N = 25;
  if(r8pid==1) fin.open("STARhomrat2/pipi1.dat");
  else if(r8pid==3) fin.open("STARhomrat2/kaka1.dat");
  else fin.open("STARhomrat2/pbarp1.dat");
  for(int i=0; i<N; i++)
    {
      fin>>pt1[i]>>ratio1[i]>>eratio[i]>>esysratio1[i];
      ept[i] = 0;
    }
  fin.close();
  TGraphErrors *tge_pipi1 = new TGraphErrors(N,pt1,ratio1,ept,eratio);
  TGraphErrors *tge_pipi1S = new TGraphErrors(N,pt1,ratio1,ept,esysratio1);


  tge_pipi->SetMarkerStyle(kFullStar);
  tge_pipi->SetMarkerStyle(kFullCross);
  ...
  tmgAuAu->Add(tge_pipi);
  tmgAuAu->Add(tge_pipi1);
  tmgAuAu->Draw("apz");
  for(int i=0; i<N; i++)
    {
      //cout<<pt1[i]<<" "<<ratio1[i]<<" "<<esysratio1[i]<<endl;
      double px1 = pt1[i]-0.05;
      double px2 = pt1[i]+0.05;
      double py1 = ratio1[i]-esysratio1[i];
      double py2 = ratio1[i]+esysratio1[i];
  ...
  for(int i=0; i<8; i++)
    {
      //cout<<pt[i]<<" "<<ratio[i]<<" "<<esysratio[i]<<endl;
      double px1 = pt[i]-0.1;
      double px2 = pt[i]+0.1;
      double py1 = ratio[i]-esysratio[i];
      double py2 = ratio[i]+esysratio[i];
      TBox *boxauaupos = new TBox(px1,py1,px2,py2);
      boxauaupos->SetFillColor(kBlack);
      boxauaupos->SetFillStyle(0);
      boxauaupos->SetLineColor(kBlack);
  ...
  legauau->AddEntry(tge_pipi,"p+p","p");
  ...
  legdau->AddEntry(tge_pipi,"p+p","p");
