void aud_ppi()
{
  ...
  ifstream fauau("../Run7AuAu/Ratios/regular/ppi_06_cent6092.txt");
  ifstream fdau("../Run8dAu/Ratios/regular/ppi_15_cent0020.txt");
  for(int i=0; i<36; i++)
    {
  ...
  TGraphErrors *tge_sysratiopos_auau = new TGraphErrors(36,pt_auau,ratiopos_auau,ept_auau,esysratiopos_auau);
  tge_sysratiopos_auau->SetMarkerColor(kGray);
  tge_sysratiopos_auau->SetLineColor(kGray);
  tge_sysratiopos_auau->SetLineWidth(15);
  TGraphErrors *tge_sysratiopos_dau = new TGraphErrors(36,pt_dau,ratiopos_dau,ept_dau,esysratiopos_dau);
  tge_sysratiopos_dau->SetMarkerColor(kGray);
  tge_sysratiopos_dau->SetLineColor(kGray);
  tge_sysratiopos_dau->SetLineWidth(15);
  TGraphErrors *tge_sysrationeg_auau = new TGraphErrors(36,pt_auau,rationeg_auau,ept_auau,esysrationeg_auau);
  tge_sysrationeg_auau->SetMarkerColor(kGray);
  tge_sysrationeg_auau->SetLineColor(kGray);
  tge_sysrationeg_auau->SetLineWidth(15);
  TGraphErrors *tge_sysrationeg_dau = new TGraphErrors(36,pt_dau,rationeg_dau,ept_dau,esysrationeg_dau);
  ...
  TGaxis *yaxis0 = new TGaxis(0.15,0.15,0.15,0.95,0.0,0.5,ndivisions,"S");
  ...
  tmg->GetYaxis()->SetTitle("Ratio p/#pi");
  ...
  tmg->GetYaxis()->SetNdivisions(ndivisions);


  //TLegend *leg = new TLegend(0.18,0.73,0.48,0.93);
  //TLegend *leg = new TLegend(0.53,0.73,0.93,0.93);
  TLegend *leg = new TLegend(0.40,0.23,0.70,0.43);
  leg->SetBorderSize(0);
  leg->SetFillColor(kWhite);
  leg->AddEntry(tge_ratiopos_auau,"p/#pi^{+} Au+Au 60-92%","p");
  leg->AddEntry(tge_rationeg_auau,"#bar{p}/#pi^{-} Au+Au 60-92%","p");
  leg->AddEntry(tge_ratiopos_dau,"p/#pi^{+} d+Au 0-20%","p");
  leg->AddEntry(tge_rationeg_dau,"#bar{p}/#pi^{-} d+Au 0-20%","p");
  leg->SetTextSize(0.05);
  leg->Draw();



  c1->Print("figures/aud_ppi.gif");
  c1->Print("figures/aud_ppi.eps");
