void dau_kpi()
{
  ...
  ifstream fdau0088("../Run8dAu/Ratios/regular/kpi_02_cent0088.txt");
  for(int i=0; i<36; i++)
    {
  ...
  ifstream fdau0020("../Run8dAu/Ratios/regular/kpi_02_cent0020.txt");
  for(int i=0; i<36; i++)
    {
  ...
  ifstream fdau2040("../Run8dAu/Ratios/regular/kpi_02_cent2040.txt");
  for(int i=0; i<36; i++)
    {
  ...
  ifstream fdau4060("../Run8dAu/Ratios/regular/kpi_02_cent4060.txt");
  for(int i=0; i<36; i++)
    {
  ...
  ifstream fdau6088("../Run8dAu/Ratios/regular/kpi_02_cent6088.txt");
  for(int i=0; i<36; i++)
    {
  ...
  float pt_pp[16];
  float ept_pp[16];
  float ratiopos_pp[16];
  float rationeg_pp[16];
  float eratiopos_pp[16];
  float erationeg_pp[16];
  float esysratiopos_pp[16];
  float esysrationeg_pp[16];
  ifstream fpp("ExtraData/PPG101/kpi.dat");
  for(int i=0; i<16; i++)
    {
      fpp>>pt_pp[i]>>ratiopos_pp[i]>>eratiopos_pp[i]>>esysratiopos_pp[i]>>rationeg_pp[i]>>erationeg_pp[i]>>esysrationeg_pp[i];
      ept_pp[i]=0;
    }
  TGraphErrors *tge_ratiopos_pp = new TGraphErrors(16,pt_pp,ratiopos_pp,ept_pp,eratiopos_pp);
  tge_ratiopos_pp->SetMarkerStyle(kFullCircle);
  tge_ratiopos_pp->SetMarkerColor(kRed);
  TGraphErrors *tge_rationeg_pp = new TGraphErrors(16,pt_pp,rationeg_pp,ept_pp,erationeg_pp);
  tge_rationeg_pp->SetMarkerStyle(kFullSquare);
  tge_rationeg_pp->SetMarkerColor(kBlue);
  TGraphErrors *tge_sysratiopos_pp = new TGraphErrors(16,pt_pp,ratiopos_pp,ept_pp,esysratiopos_pp);
  ...
  TGraphErrors *tge_sysrationeg_pp = new TGraphErrors(16,pt_pp,rationeg_pp,ept_pp,esysrationeg_pp);
  ...
  //tmgpos2->Add(tge_sysratiopos_pp);
  ...
  tmgpos2->GetXaxis()->SetLimits(0.0,4.0);
  tmgpos2->GetXaxis()->SetTitle("p_{T} (GeV/c)");
  tmgpos2->GetXaxis()->CenterTitle();
  tmgpos2->GetYaxis()->SetTitle("Ratio K/#pi");
  ...
  TLatex *tlpos = new TLatex(0.3,0.85*MAX,"K^{+}/#pi^{+}");
  ...
  legpos2->AddEntry(tge_ratiopos_dau0020,"K^{+}/#pi^{+} d+Au 0-20%","p");
  legpos2->AddEntry(tge_ratiopos_dau2040,"K^{+}/#pi^{+} d+Au 20-40%","p");
  legpos2->AddEntry(tge_ratiopos_dau0088,"K^{+}/#pi^{+} d+Au 0-88%","p");
  legpos2->AddEntry(tge_ratiopos_dau4060,"K^{+}/#pi^{+} d+Au 40-60%","p");
  legpos2->AddEntry(tge_ratiopos_dau6088,"K^{+}/#pi^{+} d+Au 60-88%","p");
  ...
  tmgneg2->GetXaxis()->SetLimits(0.0,4.0);
  tmgneg2->GetXaxis()->SetTitle("p_{T} (GeV/c)");
  tmgneg2->GetXaxis()->CenterTitle();
  tmgneg2->GetYaxis()->SetTitle("Ratio K/#pi");
  //tmgneg2->GetYaxis()->SetTitle("Ratio K^{-}/#pi^{-}");
  ...
  TLatex *tlneg = new TLatex(0.3,0.85*MAX,"K^{-}/#pi^{-}");
  ...
  legneg2->AddEntry(tge_rationeg_dau0020,"K^{-}/#pi^{-} d+Au 0-20%","p");
  legneg2->AddEntry(tge_rationeg_dau2040,"K^{-}/#pi^{-} d+Au 20-40%","p");
  legneg2->AddEntry(tge_rationeg_dau0088,"K^{-}/#pi^{-} d+Au 0-88%","p");
  legneg2->AddEntry(tge_rationeg_dau4060,"K^{-}/#pi^{-} d+Au 40-60%","p");
  legneg2->AddEntry(tge_rationeg_dau6088,"K^{-}/#pi^{-} d+Au 60-88%","p");
  ...
  c1->Print("figures/dau_kpi_both.gif");
  c1->Print("figures/dau_kpi_both.eps");
  ...
  TCanvas *c2 = new TCanvas("c2","c2",600,700);//500 looks really good with the 0.1 left margin,
                                               //but it makes the figure in the paper too big :(
  ...
  //TGaxis *yaxis1 = new TGaxis(0.15,0.5,0.15,0.925,0.0,1.2,510,"");
  TGaxis *yaxis1 = new TGaxis(0.15,0.5,0.15,0.925,0.0,MAX,505,"");
  yaxis1->SetName("yaxis1");
  yaxis1->SetTitle("Ratio K/#pi");
  yaxis1->CenterTitle();
  ...
  TGaxis *xaxis0 = new TGaxis(0.15,0.075,0.9,0.075,0.0,4.0,510,"");
  ...
  TGaxis *xaxis1 = new TGaxis(0.15,0.55,0.9,0.55,0.0,4.0,510,"");
  ...
  c2_2->cd();
  c2_2->SetLeftMargin(0.15);//old...
  //c2_2->SetLeftMargin(0.1);//new...(to match tgaxis stock...)
  ...
  tmgpos2->GetYaxis()->SetNdivisions(505); // new
  tmgpos2->Draw("apz");
  for(int i=0; i<16; i++)
    {
      double px1 = pt_pp[i]-0.05;//ept[i];
  ...
  //tlpos->Draw();
  TLatex *tlpos2 = new TLatex(3.0,0.85*MAX,"K^{+}/#pi^{+}");
  tlpos2->SetTextSize(0.075);
  tlpos2->Draw();
  TLatex *tlsys = new TLatex(0.3,0.85*MAX,"d+Au #sqrt{s_{NN}} = 200 GeV");
  tlsys->SetTextSize(0.06);
  tlsys->Draw();
  c2_1->cd();
  c2_1->SetRightMargin(0.1);
  c2_1->SetTopMargin(0.0);
  c2_1->SetLeftMargin(0.15);//old...
  //c2_1->SetLeftMargin(0.1);//new...(...)
  ...
  tmgneg2->GetYaxis()->SetNdivisions(505); // new
  tmgneg2->Draw("apz");
  for(int i=0; i<16; i++)
    {
      double px1 = pt_pp[i]-0.05;//ept[i];
  ...
  TLatex *tlneg2 = new TLatex(3.0,0.85*MAX,"K^{-}/#pi^{-}");
  tlneg2->SetTextSize(0.075);
  tlneg2->Draw();
  //tlneg->Draw();
  //TLegend *LEG = new TLegend(0.68,0.58,0.88,0.88);//adding pp now
  // TLegend *LEG = new TLegend(0.2,0.62,0.4,0.92);//adding pp now
  // LEG->SetBorderSize(0);
  ...
  TLegend *LEG = new TLegend(0.19,0.62,0.2,0.92);//adding pp now
  LEG->SetBorderSize(0);
  LEG->SetFillColor(kWhite);
  ...
  TLegend *LG2 = new TLegend(0.2,0.62,0.4,0.92);//adding pp now
  LG2->SetBorderSize(0);
  ...
  c2->Print("figures/dau_kpi_voth.gif");
  c2->Print("figures/dau_kpi_voth.eps");
